...
export default class Case extends Command {
  ...
  public async exec(
    ...
    if (id.startsWith("A-")) {
      ...
      try {
        var autoModData = await autoModModel.findOne({
          guildId: message.guild.id,
          "sanctions.caseID": id,
        });
        if (
          !autoModData ??
          autoModData.sanctions === null ??
          autoModData.sanctions.length < 1 ??
          autoModData.sanctions === undefined
        ) {
          embed.setColor(0xff0000);
          embed.setDescription(`No modlogs found for that user.`);
          return message.util.send(embed);
        } else if (!autoModData.sanctions.filter((r) => r.caseID === id)) {
          embed.setColor(0xff0000);
          embed.setDescription(`No case matching provided ID found.`);
          return message.util.send(embed);
        }
      } catch (e) {}
      const s = autoModData.sanctions.filter((r) => r.caseID === id)[0];
      embed.setAuthor(
        `Case - ${id}`,
