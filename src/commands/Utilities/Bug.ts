...
export default class Bug extends Command {
  public constructor() {
    super("bug", {
      ...
      description: {
        ...
        examples: ["bug ban Does not actually ban the member!", "bug warn Please make this more stable!"],
      },
      args: [
        {
          id: "title",
  ...
  public async exec(
    ...
    if (description.length > 1024) {
        ...
        embed.setDescription("The length of your message is above `1024` characters!");
        return message.util.send(embed);
    }
    const axisAsUser = this.client.users.cache.get(Config.bot.axis);
    if (!axisAsUser) {
        embed.setColor(0xff0000);
        embed.setDescription("Internal error with this command, report to <@!100690330336129024>.");
        return message.util.send(embed);
    }
